...
export default function SavingsPage() {
  ...
  const processedData: SavingsData[] = series.map((item, index) => {
    ...
    const cumulativeSavings = series
      .slice(0, index + 1)
      .reduce((sum, curr) => sum + curr.costSaved, 0);
    const monthNames = [
      "Jan",
      "Feb",
      "Mar",
  ...
  const averageROI =
    series.length > 0
      ? ((totalCostSaved - totalPlanCost) / totalPlanCost) * 100
      : 0;
              ...
              <span className="text-sm font-medium text-gray-900 dark:text-white">
                {entry.dataKey === "roi"
                  ...
                  : entry.dataKey === "costSaved" ||
                      entry.dataKey === "cumulativeSavings"
                    ? `$${entry.value.toLocaleString()}`
                    : entry.dataKey === "timeSaved"
                      ? `${entry.value} hours`
                      : entry.value}
              </span>
            </div>
  ...
  return (
    <div className="space-y-8">
      <PageTitle
        title="Savings & ROI"
        subtitle="Analyzing your cost savings and return on investment"
      />
