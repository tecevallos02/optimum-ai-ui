...
export async function generateSavingsPdf(data: any[]): Promise<Blob> {
  ...
  data.forEach((item, index) => {
    ...
    // Calculate ROI
    const roi = item.planCost > 0 ? ((item.costSaved - item.planCost) / item.planCost * 100).toFixed(1) : '0';
  ...
  const overallROI = totalPlanCost > 0 ? ((totalCostSaved - totalPlanCost) / totalPlanCost * 100).toFixed(1) : '0';
