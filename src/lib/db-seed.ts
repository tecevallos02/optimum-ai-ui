...
async function main() {
  ...
  const organizations = [
    ...
    },
    {
      ...
      description:
        "Full-service law firm with expertise in business and real estate",
  ...
  for (const userData of users) {
    ...
    console.log(
      `✅ Created user: ${user.name} (${user.email}) as ${userData.role} in ${org.name}`,
    );
  }
    ...
    if (users.length > 0) {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          actorId: user.id,
          action: action,
          target: action.includes("Contact")
            ? "contact"
  ...
  console.log(
    "   - partner@greenvalleylaw.com (Owner of Green Valley Law Firm)",
  );
  console.log(
    "   - admin@sunrisemedical.com (Owner of Sunrise Medical Center)",
  );
