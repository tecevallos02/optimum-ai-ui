...
export function generateMockDataForCompany(
  companyId: string,
  companyName: string,
) {
  ...
  const baseData: CallRow[] = [
    ...
    },
    {
      ...
      status: "completed",
      notes:
        "Old furnace replaced with high-efficiency model, significant energy savings expected",
      intent: "replacement",
    },
  ];
  ...
  return baseData.map((call, index) => ({
    ...
    address: call.address.replace(
      "New York, NY",
      getCompanyLocation(companyName),
    ),
...
function getServiceTypeForCompany(
  companyName: string,
  defaultService: string,
): string {
  if (
    companyName.includes("Air Conditioning") ||
    companyName.includes("Cooling")
  ) {
